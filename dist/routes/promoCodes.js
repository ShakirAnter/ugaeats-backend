...
router.get("/", async (req, res) => {
    try {
        const promos = await PromoCode_1.PromoCode.find({
            is_active: true,
 ...
 */
router.post("/validate", auth_1.auth, async (req, res) => {
    try {
 ...
 */
router.post("/apply/:promoId", auth_1.auth, async (req, res) => {
    try {
        const { promoId } = req.params;
        const promo = await PromoCode_1.PromoCode.findById(promoId);
        if (!promo)
