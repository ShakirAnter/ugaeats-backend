 ...
 */
router.post("/item/:itemId", auth, async (req: any, res) => {
  try {
  ...
  try {
    ...
    if (orderId) {
      ...
      if (existingReview) {
        ...
        if (rider_rating) existingReview.rider_rating = rider_rating;
        if (rider_comment) existingReview.rider_comment = rider_comment;
